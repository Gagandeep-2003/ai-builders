...
export function ResourceLibrary({
  ...
  const availableModuleDecks = useMemo(() => {
    const unlockedSessions = getUnlockedSessions(sessions);

    return moduleDecks
      .map((deck) => {
        const courseModule = modules.find((module) => module.orderIndex === deck.moduleNumber);
        const unlockedSessionNumbers = new Set(
          unlockedSessions
            .filter((session) => session.moduleId === courseModule?.id)
            .map((session) => session.sessionNumber),
        );

        return {
          ...deck,
          sessions: deck.sessions.filter((session) => unlockedSessionNumbers.has(session.sessionNumber)),
        };
      })
      .filter((deck) => deck.sessions.length > 0);
  }, [modules, sessions]);

  const [activeModule, setActiveModule] = useState(
    availableModuleDecks[0]?.moduleNumber ?? 1,
  );
  const [activeSession, setActiveSession] = useState(
    availableModuleDecks[0]?.sessions[0]?.sessionNumber ?? 1,
  );
  const [expandedSession, setExpandedSession] = useState<ExpandedSessionDeck | null>(null);

  const selectedModule =
    availableModuleDecks.find((module) => module.moduleNumber === activeModule) ??
    availableModuleDecks[0];
  ...
  return (
    <div className="space-y-6">
      <section className="grid gap-4 md:grid-cols-3">
        {availableModuleDecks.map((module) => {
          ...
          return (
            <button
              ...
              onClick={() => {
                ...
                setActiveSession(module.sessions[0]?.sessionNumber ?? 1);
              }}
              className={cn(
      ...
      {!selectedModule ? (
        <ComingSoonPanel moduleNumber={1} title="Resource Library" />
