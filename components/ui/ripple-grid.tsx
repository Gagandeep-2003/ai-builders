...
export function RippleGrid({
  ...
  useEffect(() => {
    ...
    const handleMouseEnter = () => {
      if (mouseInteraction) mouseInfluenceRef.current = 1;
    };

    const handleMouseLeave = () => {
      if (mouseInteraction) mouseInfluenceRef.current = 0;
    };
    ...
    const render = (time: number) => {
      ...
      uniforms.mouseInfluence.value =
        Number(uniforms.mouseInfluence.value) + (mouseInfluenceRef.current - Number(uniforms.mouseInfluence.value)) * 0.05;
    ...
    // WebGL setup is intentionally mounted once; prop updates flow through the uniform update effect below.
  ...
  return <div ref={containerRef} className={`ripple-grid-container h-full w-full ${className}`} />;
}
